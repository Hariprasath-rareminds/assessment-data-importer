...
Output:
  - output/seeds/<email_local_part>_assessment_seed.sql
...
FORM_SHEETS = {
    ...
    "mca domain ": "mca_knowledge",
    "mca apt": "mca_aptitude",
}
...
def repair_mojibake(text: str) -> str:
    ...
    # Question-bank exports can contain UTF-8 text decoded as Windows-1252.
    # Repair that only when mojibake markers are present.
...
def alpha_question_key(value: Any) -> str:
    """Very conservative prose-only key used only as a final AI-question fallback."""
    s = repair_mojibake(as_text(value))
...
def prose_question_key(value: Any) -> str:
    """Letters-only fallback key for passage wrappers and visual-symbol mojibake."""
    s = repair_mojibake(as_text(value))
    s = unicodedata.normalize("NFKC", s).lower()
    s = re.sub(r"\bread the passage and answer the question\b", " ", s)
    s = re.sub(r"\bread the passage\b", " ", s)
    s = re.sub(r"\b(?:passage|question)\b", " ", s)
    s = re.sub(r"[^a-z ]+", " ", s)
    return re.sub(r"\s+", " ", s).strip()
    ...
    for row in workbook_rows(wb["career_assessment_ai_questions"]):
        ...
        for item in json_load(row.get("questions"), []):
            ...
            q = AiQuestion(
                ...
                stream_id=as_text(row.get("stream_id")) or None,
            )
...
def find_email(row: Dict[str, Any]) -> str:
    for key in ("Email Address", "Email ID", "Email Id", "Email"):
...
def load_form_students(path: Path) -> Tuple[Dict[str, Dict[str, Any]], Dict[str, List[str]]]:
    wb = load_workbook(path, read_only=True, data_only=True)
    ...
    for sheet_name, stage in FORM_SHEETS.items():
        ...
        expected_headers[stage] = [
            as_text(h) for h in first_row
            if as_text(h) and as_text(h).strip().lower() not in IDENTITY_HEADERS
            and as_text(h).strip().lower() != "specialization"
        ]
        for row in workbook_rows(ws):
    ...
    return students, expected_headers


def detect_program(student: Dict[str, Any]) -> Optional[str]:
    stream = as_text(student.get("stream")).upper()
    stages = student.get("stages", {})
    if "MCA" in stream or "mca_knowledge" in stages or "mca_aptitude" in stages:
        return "MCA"
    if "MBA" in stream or "mba_knowledge" in stages or "mba_aptitude" in stages:
        return "MBA"
    return None


def resolve_ai_question(
    header: str,
    lookup: Dict[str, List[AiQuestion]],
    aliases: Dict[str, str],
    program: str,
    expected_type: str,
) -> Optional[AiQuestion]:
    def eligible(items: Iterable[AiQuestion]) -> List[AiQuestion]:
        result = []
        for q in items:
            stream = (q.stream_id or "").strip().lower()
            qtype = (q.question_type or "").strip().lower()
            program_ok = (program == "MCA" and stream == "mca") or (program == "MBA" and stream in {"mba", "master of business administration"})
            if program_ok and qtype == expected_type:
                result.append(q)
        return result

    key = normalize_question(header)
    candidates = eligible(lookup.get(key, []))
    if len(candidates) == 1:
        return candidates[0]

    aliased = aliases.get(key)
    if aliased:
        candidates = eligible(lookup.get(aliased, []))
        if len(candidates) == 1:
            return candidates[0]

    # Final fallback for encoding-damaged visual/symbol questions. Match prose
    # within the same program + question type only.
    alpha = alpha_question_key(header)
    matches: Dict[str, AiQuestion] = {}
    eligible_all: Dict[str, AiQuestion] = {}
    for items in lookup.values():
        for q in eligible(items):
            eligible_all[q.id] = q
            if alpha_question_key(q.text) == alpha:
                matches[q.id] = q
    if len(matches) == 1:
        return next(iter(matches.values()))

    # Passage wrappers and damaged visual-symbol encodings can leave extra text
    # or mojibake digits. Use unique prose containment only within the already
    # filtered program/type set.
    contained: Dict[str, AiQuestion] = {}
    for qid, q in eligible_all.items():
        q_alpha = alpha_question_key(q.text)
        if min(len(alpha), len(q_alpha)) >= 20 and (alpha in q_alpha or q_alpha in alpha):
            contained[qid] = q
    if len(contained) == 1:
        return next(iter(contained.values()))

    prose = prose_question_key(header)
    prose_matches: Dict[str, AiQuestion] = {}
    for qid, q in eligible_all.items():
        q_prose = prose_question_key(q.text)
        if min(len(prose), len(q_prose)) >= 20 and (prose == q_prose or prose in q_prose or q_prose in prose):
            prose_matches[qid] = q
    return next(iter(prose_matches.values())) if len(prose_matches) == 1 else None


PRODUCTION_STAGE_SHEETS = {
    "Eligible - Big5": "bigfive",
    "Eligible - RIASEC": "riasec",
    "Eligible - Employability": "employability",
    "Eligible - Work Values": "values",
    "Eligible - General Aptitude": "adaptive_aptitude",
    "Eligible - MBA Domain": "mba_knowledge",
    "Eligible - MBA Aptitude": "mba_aptitude",
    "Eligible - MCA Domain": "mca_knowledge",
    "Eligible - MCA Aptitude": "mca_aptitude",
}


def load_production_data(path: Optional[Path]) -> Tuple[Dict[str, Dict[str, str]], Dict[str, Dict[str, Dict[str, Any]]]]:
    """Load eligibility policy + cleaned eligible rows from FINAL_PRODUCTION data."""
    policy: Dict[str, Dict[str, str]] = {}
    cleaned: Dict[str, Dict[str, Dict[str, Any]]] = defaultdict(dict)
    if not path or not path.exists():
        return policy, cleaned

    wb = load_workbook(path, read_only=True, data_only=True)

    if "Eligible Students" in wb.sheetnames:
        for row in workbook_rows(wb["Eligible Students"]):
            ...
            policy[email] = {
                "status": "ELIGIBLE",
                "reason": "",
                "program": as_text(row.get("Program")).upper(),
            }

    if "Not Eligible Students" in wb.sheetnames:
        for row in workbook_rows(wb["Not Eligible Students"]):
            email = as_text(row.get("Email")).lower()
            if not email or "@" not in email:
                continue
            reason = as_text(row.get("Primary Reason"))
            program = as_text(row.get("Program")).upper()
            if reason == "Conflicting Duplicate Answer":
                # New migration rule: keep first answer and allow seed generation.
                status = "ALLOW_FIRST_DUPLICATE"
            else:
                status = "BLOCK"
            policy[email] = {"status": status, "reason": reason, "program": program}

    # Cleaned rows are authoritative for students already marked eligible.
    for sheet_name, stage in PRODUCTION_STAGE_SHEETS.items():
        if sheet_name not in wb.sheetnames:
            continue
        for row in workbook_rows(wb[sheet_name]):
            email = find_email(row) or as_text(row.get("Email")).lower()
            if email:
                cleaned[email][stage] = row

    return policy, cleaned


def apply_cleaned_rows(
    students: Dict[str, Dict[str, Any]],
    cleaned: Dict[str, Dict[str, Dict[str, Any]]],
    policy: Dict[str, Dict[str, str]],
) -> None:
    for email, stages in cleaned.items():
        if email not in students:
            continue
        if policy.get(email, {}).get("status") != "ELIGIBLE":
            continue
        students[email]["stages"].update(stages)
        if policy[email].get("program"):
            students[email]["stream"] = policy[email]["program"]


def required_stages_for_program(program: str) -> List[str]:
    common = ["bigfive", "riasec", "employability", "values", "adaptive_aptitude"]
    if program == "MCA":
        return common + ["mca_knowledge", "mca_aptitude"]
    return common + ["mba_knowledge", "mba_aptitude"]
...
def convert_student(
    ...
    expected_headers: Dict[str, List[str]],
    production_policy: Optional[Dict[str, str]] = None,
    ...
    def warn(stage: str, question: str, message: str):
        warnings.append({"email": email, "stage": stage, "question": question, "message": message})

    program = detect_program(student)
    if not program:
        err("identity", "program", "program could not be determined as MBA or MCA")
        return None, errors, warnings

    # FINAL_PRODUCTION classification is authoritative when supplied.
    # Conflicting duplicates are explicitly allowed; missing/incomplete and
    # identity/email issues remain blocking.
    if production_policy:
        status = production_policy.get("status")
        reason = production_policy.get("reason") or ""
        if status == "BLOCK":
            err("validation", "", f"{reason}: seed generation blocked")
            return None, errors, warnings
        if status == "ALLOW_FIRST_DUPLICATE":
            warn("validation", "", "Conflicting Duplicate Answer: first answer will be kept")
        policy_program = (production_policy.get("program") or "").upper()
        if policy_program in {"MBA", "MCA"}:
            program = policy_program

    # Always require all seven program-appropriate stages to exist.
    # We intentionally do NOT compare against every question in the bank because
    # the production MCA Aptitude form contains 49 items while the bank has 50.
    for stage in required_stages_for_program(program):
        if not student["stages"].get(stage):
            err(stage, "", "Missing/Incomplete Assessment: stage missing")
    ...
    for stage in ("bigfive", "riasec", "values"):
        ...
        for header, answer in row.items():
            ...
            if q.id in seen:
                if seen[q.id] != score:
                    warn(stage, header, f"Conflicting Duplicate Answer: kept first answer {seen[q.id]}, ignored {score}")
                continue
            seen[q.id] = score
    ...
    if row:
        ...
        for header, answer in row.items():
            ...
            if kind:
                ...
                if kind in sjt[q.id]:
                    if sjt[q.id][kind] != val:
                        warn("employability", header, f"Conflicting Duplicate Answer: kept first SJT {kind} answer")
                    continue
                sjt[q.id][kind] = val
            else:
                key = normalize_answer(answer).lower()
    ...
    # Program-specific domain knowledge + aptitude come ONLY from the
    # career_assessment_ai_questions sheet in question_bank.xlsx.
    ai_stages = ("mca_knowledge", "mca_aptitude") if program == "MCA" else ("mba_knowledge", "mba_aptitude")
    for stage in ai_stages:
        row = student["stages"].get(stage)
        expected_type = "knowledge" if stage.endswith("knowledge") else "aptitude"
        for header, answer in row.items():
            if header.strip().lower() in IDENTITY_HEADERS or header.strip().lower() == "specialization" or answer in (None, ""):
                continue
            q = resolve_ai_question(header, ai_lookup, ai_aliases, program, expected_type)
            if not q:
                err(stage, header, "AI question not mapped in question_bank.xlsx/career_assessment_ai_questions")
                continue
            stored = ai_answer_for_storage(answer, q.options)
            if q.id not in all_responses:
                all_responses[q.id] = stored
            elif all_responses[q.id] != stored:
                warn(stage, header, "Conflicting Duplicate Answer: kept first mapped AI answer")
...
def main() -> int:
    ...
    parser.add_argument("--ai-sql", default=None, help="Deprecated; ignored. AI questions come only from question_bank.xlsx")
    parser.add_argument("--production-data", default=None, help="FINAL_PRODUCTION_ASSESSMENT_DATA_v3.xlsx; controls eligibility and supplies cleaned eligible rows")
    ...
    # --ai-sql is retained only for backward-compatible CLI usage.
    # AI/domain questions are loaded exclusively from question_bank.xlsx.
    static_aliases = build_alias_index(static_lookup.keys())
    ai_aliases = build_alias_index(ai_lookup.keys())
    students, expected_headers = load_form_students(answers)
    production_path = Path(args.production_data) if args.production_data else None
    production_policy, cleaned_rows = load_production_data(production_path)
    apply_cleaned_rows(students, cleaned_rows, production_policy)

    valid: List[Dict[str, Any]] = []
    all_errors: List[Dict[str, str]] = []
    all_warnings: List[Dict[str, str]] = []
    for email, student in sorted(students.items()):
        data, errors, warnings = convert_student(
            email, student, static_lookup, static_by_id, ai_lookup, static_aliases, ai_aliases, expected_headers, production_policy.get(email)
        )
    ...
    with report_path.open("w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["severity", "email", "stage", "question", "message"])
        writer.writeheader()
        writer.writerows([{**row, "severity": "ERROR"} for row in all_errors])
        writer.writerows([{**row, "severity": "WARNING"} for row in all_warnings])
    ...
    print(f"Blocking validation issues: {len(all_errors)}")
    print(f"Duplicate warnings (first answer kept): {len(all_warnings)}")
    print(f"Report: {report_path}")
    if production_path:
        print(f"Production policy: {production_path}")
